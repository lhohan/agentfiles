...
const COOKIE_DISMISS_SCRIPT = `(acceptCookies) => {
 ...
 if (text.includes('cookie') && text.length > 100 && text.length < 5000) {
 const btn = findButtonByText(patterns, container);
 if (btn) { btn.click(); clicked.push('Generic (text heuristic)'); return clicked; }
 }
 }

 // Absolute last resort: find any visible button matching accept/reject text
 const buttons = Array.from(document.querySelectorAll('button, [role="button"]'));
 const singleWordPatterns = acceptCookies
 ? ['accept', 'agree', 'allow', 'ok', 'got it']
 : ['reject', 'decline', 'deny'];
