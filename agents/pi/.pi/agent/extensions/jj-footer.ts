...
function formatCount(value: number): string {
	...
	const formatted = unitIndex === 0 ? `${scaled}` : `${scaled.toFixed(1)}${units[unitIndex]}`;
...
function formatHomePath(pathname: string): string {
	...
	if (pathname.startsWith(`${home}/`)) return `~/${pathname.slice(home.length + 1)}`;
	return pathname;
}

function alignLeftRight(left: string, right: string, width: number, ellipsis = ""): string {
...
function classifyJjStatusLine(line: string): "added" | "changed" | "deleted" | "conflicted" | null {
...
function parseJjStatusCounts(stdout: string): FileCounts {
	const counts: FileCounts = { added: 0, changed: 0, deleted: 0, conflicted: 0 };
...
async function readJjBookmarkInfo(pi: ExtensionAPI, cwd: string): Promise<JjBookmarkInfo> {
	const template = 'commit_id ++ "\\n" ++ bookmarks.map(|b| b.name()).join("\\n")';
	const result = await pi.exec(
		"jj",
		["log", "-r", "latest(heads(::@ & bookmarks()))", "--ignore-working-copy", "--no-graph", "--no-pager", "--color=never", "-T", template],
		{ cwd, timeout: 2500 },
	);
	...
	const aheadResult = await pi.exec(
		"jj",
		["log", "-r", `${anchor.commitId}..@`, "--ignore-working-copy", "--no-graph", "--no-pager", "--color=never", "-T", 'commit_id ++ "\\n"'],
		{ cwd, timeout: 2500 },
	);
	const ahead = (aheadResult.stdout ?? "").split(/\r?\n/).filter((line) => line.trim().length > 0).length;

	return { bookmark: anchor.bookmark, ahead };
}

async function readFileCounts(pi: ExtensionAPI, cwd: string): Promise<FileCounts> {
	const result = await pi.exec("jj", ["status", "--quiet", "--no-pager", "--color=never"], {
		cwd,
		timeout: 2500,
	});
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", (_event, ctx) => {
		...
		let fileCounts: FileCounts = { added: 0, changed: 0, deleted: 0, conflicted: 0 };
		let disposed = false;
		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					const contextPercent = contextUsage?.percent !== null ? Math.round(contextPercentValue).toString() : "?";
					const contextPercentDisplay = contextPercent === "?" ? `ctx ?` : `ctx ${contextPercent}%`;
					...
					if (output) statsParts.push(theme.fg("dim", `↓${formatCount(output)}`));
					if (cacheRead) statsParts.push(theme.fg("dim", `-${formatCount(cacheRead)}`));
					statsParts.push(theme.fg("dim", `$${cost.toFixed(2)}${billingSuffix}`));
					...
					const location = theme.fg("dim", truncateToWidth(formatHomePath(ctx.cwd), width, "…"));
					...
					const bookmarkColor = jjInfo.bookmark.startsWith("(") ? "muted" : "accent";
					const bookmarkText =
						jjInfo.ahead > 0
							? `${theme.fg(bookmarkColor, jjInfo.bookmark)}${theme.fg("muted", `+${formatCount(jjInfo.ahead)}`)}`
							: theme.fg(bookmarkColor, jjInfo.bookmark);
					const line1 = alignLeftRight(location, `${fileCountsText} ${bookmarkText}`, width);
