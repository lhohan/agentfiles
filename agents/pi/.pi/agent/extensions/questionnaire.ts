/**
 * Questionnaire Tool - Unified tool for asking single or multiple bounded questions.
 *
 * Single question: simple options list
 * Multiple questions: tab bar navigation between questions
 *
 * Adapted from Pi's upstream questionnaire example for this repository's
...
export default function questionnaire(pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "Questionnaire",
		description:
			"Ask the user one or more bounded clarification questions with explicit options. Use for clarifying requirements, getting preferences, or confirming decisions when prose questions would create avoidable back-and-forth. For single questions, shows a simple option list. For multiple questions, shows a tab-based interface.",
					...
					function renderOptions() {
						for (let i = 0; i < opts.length; i++) {
							...
							const labelText = isOther && inputMode
								? `${i + 1}. ${opt.label} ✎`
								: `${i + 1}. ${opt.label}`;
							addWrappedWithPrefix(prefix, theme.fg(color, labelText));
							if (opt.description) {
					...
					if (inputMode && q) {
						...
						for (const question of questions) {
							...
							if (answer) {
								...
								addWrappedWithPrefix(" ", theme.fg("muted", `${question.label}: `) + theme.fg("text", prefix + answer.label));
							}
						}
						lines.push("");
						if (allAnswered()) {
