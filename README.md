...
- A [Usage statistics extension](agents/pi/.pi/agent/extensions/usage-stats.ts) that tracks skill, prompt, loaded extensions, command, tool, and model usage into a local JSONL file, with standalone reporting executables at [`~/.pi/agent/bin/pi-usage-report`](agents/pi/.pi/agent/bin/pi-usage-report) and [`~/.pi/agent/bin/pi-usage-report-html`](agents/pi/.pi/agent/bin/pi-usage-report-html). The reports include consistent most-used, least-used, and full-list artifact rankings for prompts, skills, custom tools, and enabled models, with an HTML at-a-glance summary. [Documentation](agents/pi/.pi/agent/extensions/usage-stats.md)
