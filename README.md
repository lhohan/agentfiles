...
- A [Usage statistics extension](agents/pi/.pi/agent/extensions/usage-stats.ts) that tracks skill, prompt, command, and model usage into a local JSONL file, with a standalone [viewer script](agents/pi/.pi/agent/extensions/usage-stats-viewer.mjs). [Documentation](agents/pi/.pi/agent/extensions/usage-stats.md)
